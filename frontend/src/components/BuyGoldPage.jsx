...
export default function BuyGoldPage({ onBack }) {
  ...
  // 24K Buy rate calculation (usually buy rate is slightly higher than sell rate by ~3% spread)
  const buyRate24k = Math.round((liveRate14k * 24 / 14) * 1.03);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setSecondsLeft((prev) => {
        if (prev <= 1) {
          return Math.floor(Math.random() * 30) + 60; // reset
        }
        return prev - 1;
      });
    }, 1000);
  ...
  const handleAmountChange = (val) => {
    ...
    const calculatedGrams = (parseFloat(val) / buyRate24k).toFixed(4);
  ...
  const handleGramsChange = (val) => {
    ...
    const calculatedAmount = Math.round(parseFloat(val) * buyRate24k);
  ...
  const faqs = [
    {
      ...
      answer: "Enter the amount in Rupees (minimum ₹100) or weight in grams you wish to buy. Complete the payment securely via UPI, card, or net banking. Once successful, the equivalent weight of certified 24K gold is added to your secure digital balance vault instantly."
    },
    {
      question: "Is the gold I buy secure?",
      answer: "Yes, 100%. The gold you buy is backed by real, physical 24K gold of 99.99% purity, stored securely inside insured vaults with Brink's, and monitored by an independent SEBI-regulated trustee."
    },
    {
      question: "What is the minimum buy amount?",
      answer: "You can purchase digital gold starting with an amount as low as ₹100, making it convenient to invest systematically."
    },
    {
      question: "Can I get physical delivery of my gold?",
      answer: "Absolutely! You can request physical delivery of your gold balance in the form of certified gold coins or bars, or you can exchange your digital credits for physical ornaments at any of our branches."
    }
  ];
  ...
  return (
    <div className="buygold-page-container">
      {/* Back button & Breadcrumb */}
      <div className="buygold-header-nav">
        <button className="buygold-back-btn" onClick={handleBackToHome}>
          <ArrowLeft size={16} /> Back to Home
        </button>
        <span className="buygold-breadcrumb">
          <span style={{ cursor: 'pointer' }} onClick={handleBackToHome}>Home</span> / Digital Gold / Buy Gold
        </span>
      </div>
      ...
      <div className="buygold-content-wrapper">
        <h1 className="buygold-main-title">Buy Gold</h1>

        {/* 3-Card Grid */}
        <div className="buygold-cards-grid">
          {/* Card 1: Converter Form */}
          <div className="buygold-card convert-card">
            <form onSubmit={handleProceed}>
              <div className="converter-flex">
                <div className="convert-input-group">
                  <label className="convert-label">Buy Gold by Amount</label>
                ...
                <div className="convert-input-group">
                  <label className="convert-label">or Buy in Grams</label>
                  <div className="convert-input-wrap">
                    <input
                      type="number"
                      step="0.0001"
          ...
          <div className="buygold-card rate-card">
            <div className="rate-card-header">
              <span className="card-lbl">Buy Rate</span>
              <span className="timer-badge">
                <Clock size={12} className="timer-icon" /> Price valid for {formatTimer(secondsLeft)} min
              </span>
            </div>
            <div className="rate-value">₹{buyRate24k.toLocaleString('en-IN')}/gram</div>
          ...
          <div className="buygold-card balance-card">
        ...
        <div className="buygold-faq-section">
          <h2 className="faq-main-title">Buying the Gold <span>FAQs</span></h2>
      ...
      <style>{`
        ...
        .buygold-submit-btn:hover:not(:disabled) {
          background-color: #8C735B;
          transform: translateY(-1px);
        }

        .buygold-submit-btn:disabled {
