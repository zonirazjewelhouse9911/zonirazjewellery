...
export default function Banners() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    img.onload = async () => {
      ...
      // Validate dimensions: max width 1396 (allowing up to 1398 for pixel rounding), max height 480
      if (img.width > 1398 || img.height > 480) {
        setValidationError(`Image dimensions (${img.width}x${img.height}px) exceed the maximum permitted size of 1396 x 480 px.`);
              ...
              <div className="flex items-start gap-2 bg-[#efe7e5]/50 border border-slate-200/50 rounded-xl p-3 mt-2">
                ...
                <div>
                  ...
                  <span className="text-[9px] text-slate-500 block leading-normal mt-0.5">
                    Images must be at most <strong>1396 x 480 px</strong> to maintain premium layout alignment.
                  </span>
                </div>
              </div>
            </div>
