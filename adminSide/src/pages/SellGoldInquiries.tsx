...
import { Loader2, Search, MapPin, Clock, FileText, X, Check, Phone, Mail, HelpCircle, AlertTriangle, Layers, Info } from 'lucide-react';

interface SellGoldInquiry {
  _id: string;
  fullName: string;
  phone: string;
  email: string;
...
export default function SellGoldInquiries() {
  const [inquiries, setInquiries] = useState<SellGoldInquiry[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');

  // Modal detail state
  const [selectedInquiry, setSelectedInquiry] = useState<SellGoldInquiry | null>(null);
  ...
  const fetchInquiries = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/sell-gold-inquiries');
      const data = await res.json();
      if (data.success) {
        setInquiries(data.data || []);
      } else {
        console.error('Failed to fetch sell gold leads:', data.message);
      }
    } catch (error) {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/sell-gold-inquiries/${id}/status`, {
        method: 'PATCH',
  ...
  const getContactIcon = (method: string) => {
    switch (method) {
      ...
      case 'whatsapp':
        return (
          <span className="text-slate-400 shrink-0 font-bold font-sans text-xs">WA</span>
        );
      case 'email':
  ...
  const filteredInquiries = inquiries.filter(inq => {
    ...
    const branch = inq.branch.toLowerCase();
    const query = searchQuery.toLowerCase();

    const matchesSearch = name.includes(query) || email.includes(query) || phone.includes(query) || city.includes(query) || branch.includes(query);
    const matchesStatus = statusFilter === 'all' || inq.status === statusFilter;

    return matchesSearch && matchesStatus;
  });

  const handleOpenDetailModal = (inq: SellGoldInquiry) => {
              ...
              <div key={inq._id} className="p-6 grid grid-cols-12 gap-4 items-center hover:bg-slate-50/30 transition-colors">
                ...
                <div className="col-span-3 flex items-center space-x-4 min-w-0">
                  ...
                  <div className="min-w-0 text-left">
                    ...
                    <span className="text-[11px] text-slate-400 truncate block mt-0.5 select-all">{inq.phone}</span>
                  </div>
                </div>

                {/* Location */}
                <div className="col-span-3 text-left space-y-1 min-w-0">
                  <div className="flex items-center space-x-2 text-xs text-slate-850">
                    <MapPin size={13} className="text-slate-400 shrink-0" />
                    <span className="truncate">{inq.city || 'Unknown'}</span>
                  </div>
                  <div className="text-[9px] text-slate-400 font-black uppercase tracking-[0.15em] pl-5">
                    {inq.branch ? `${inq.branch.toUpperCase()} BRANCH` : 'ALWAR BRANCH'}
                  </div>
                </div>
                ...
                <div className="col-span-3 text-left space-y-1">
                  <div className="text-xs font-bold text-slate-800">
                    {inq.jewelleryType} &middot; {inq.approximateWeight}g
                  </div>
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                    {inq.purity || 'N/A'} {inq.knowsPurity ? '(Verified)' : '(Unverified)'}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6 backdrop-blur-xs animate-in fade-in duration-300">
          <div className="bg-[#efe7e5] text-[#12100e] w-full max-w-xl rounded-4xl shadow-premium border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 bg-white border-b border-slate-200/60 flex items-center justify-between shrink-0">
              <div>
                <span className="text-[9px] uppercase tracking-[0.3em] font-black text-slate-400">Sell Gold Lead File</span>
                <h3 className="text-xl font-serif font-bold italic mt-1 text-[#12100e]">Evaluation Profile</h3>
              </div>
              <button 
                type="button"
            ...
            <div className="p-8 space-y-6 text-left overflow-y-auto flex-1">
              ...
              <div className="bg-white rounded-3xl p-6 border border-slate-200/60 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100 text-xs text-slate-650">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span>{selectedInquiry.city} &middot; {selectedInquiry.branch} Branch</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    {getContactIcon(selectedInquiry.preferredContactMethod)}
                    <span className="capitalize">Contact by {selectedInquiry.preferredContactMethod}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Jewelry Details */}
              <div className="bg-white rounded-3xl p-6 border border-slate-200/60 space-y-4">
                <h4 className="text-[10px] uppercase tracking-widest font-black text-[#a88265] flex items-center gap-2">
                  <Layers size={12} />
                  <span>Item Evaluation Specifications</span>
                </h4>
                
                <div className="grid grid-cols-3 gap-4 text-center bg-slate-50/50 rounded-2xl p-4 border border-slate-100 text-xs">
                  <div>
                    <span className="text-[8px] uppercase tracking-widest text-slate-400 block font-black">Type</span>
                    <strong className="text-slate-850 block mt-1 capitalize">{selectedInquiry.jewelleryType || 'N/A'}</strong>
                  </div>
                  <div>
                    <span className="text-[8px] uppercase tracking-widest text-slate-400 block font-black">Weight</span>
                    <strong className="text-slate-850 block mt-1">{selectedInquiry.approximateWeight}g</strong>
                  </div>
                  <div>
                    <span className="text-[8px] uppercase tracking-widest text-slate-400 block font-black">Karat</span>
                    <strong className="text-slate-850 block mt-1">{selectedInquiry.purity || 'N/A'}</strong>
                  </div>
                </div>

                {selectedInquiry.notes && (
                  <div className="pt-2 text-xs leading-relaxed text-slate-500 border-t border-slate-100">
                    <span className="text-[8px] uppercase tracking-widest text-slate-400 block font-black mb-1">Customer Notes</span>
                    <p className="bg-slate-50/65 rounded-xl p-3 border border-slate-150/40 italic">{selectedInquiry.notes}</p>
                  </div>
                )}

                {selectedInquiry.preferredVisitDate && (
                  <div className="pt-2 text-xs leading-relaxed text-slate-500 border-t border-slate-100 flex items-center space-x-2">
                    <Info size={13} className="text-slate-400 shrink-0" />
                    <span>Preferred showroom visit date: <strong>{formatDate(selectedInquiry.preferredVisitDate)}</strong></span>
                  </div>
                )}
              </div>
